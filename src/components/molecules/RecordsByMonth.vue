<template>
  <div class="records-by-month">
    <div class="records-by-month-title">{{ japaneseThisYearMonth }}の合計記録</div>
    <input-month
      class="records-by-month-date-select"
      :value-text="selectedDate"
      @input="displayAmountsBy"
    ></input-month>
    <div class="records-by-month-pure-alcohol">純アルコール量 : {{ totalPureAlcohol }} g</div>
    <div class="records-by-month-drinking-amount">飲酒量 : {{ totalDrinking }} ml</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    setNumericThisYearMonth() {
      ...
      const yearMonth = today.getFullYear().toString() + "-" + (today.getMonth() + 1).toString();
      this.numericThisYearMonth = this.covertNumericDateToMonthWithZero(yearMonth)
    },
