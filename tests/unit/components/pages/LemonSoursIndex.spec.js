...
describe("(pc display) LemonSoursIndex component test", () => {
  ...
  describe("レモンサワーのデータを検索するとき、", () => {
    it("データが存在するとき、該当するレモンサワーのデータが返ってくる", async () => {
      ...
      expect(wrapper.find(".sour-image").attributes("src")).toBe("@/assets/test/ls_test_sample2.png");
    });
    ...
    it("データが存在しないとき、エラーメッセージが返ってくる", async () => {
      ...
      expect(wrapper.find(".pc-sours-index-items-error").text()).toBe("そんなデータあらへん");
    });
  })
});
...
describe("(sp display) LemonSoursIndex component test", () => {
  ...
  describe("レモンサワーのデータを検索するとき、", () => {
    ...
    it("データが存在しないとき、エラーメッセージが返ってくる", async () => {
      ...
      expect(wrapper.find(".sp-sours-index-items-error").text()).toBe("そんなデータあらへん");
    });
  });
});
